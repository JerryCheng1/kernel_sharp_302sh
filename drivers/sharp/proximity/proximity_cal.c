...
static atomic_t     updata_flag = ATOMIC_INIT(0);
static atomic_t     dataread_func_flag = ATOMIC_INIT(0);  /* 0=Disable,1=Enable */
static atomic_t     prox_irq_disable_count = ATOMIC_INIT(1);
static atomic_t     prox_irq_enable_wake_count = ATOMIC_INIT(0);
...
//          prox_subscribe();
        ...
        nResult = shdisp_api_prox_sensor_pow_ctl(SHDISP_PROX_SENSOR_BGMODE_ON,NULL);
        if(nResult != SHDISP_RESULT_SUCCESS) {
        ...
        if(atomic_read(&sensor_data) == 0){ /*Near*/
        ...
        atomic_inc( &prox_irq_enable_wake_count );
        DEBUG_LOG("enable_wake_count = %d",atomic_read(&prox_irq_enable_wake_count));

        while( 0 < atomic_read(&prox_irq_disable_count) ){
	    DEBUG_LOG("irq_disable_count = %d",atomic_read(&prox_irq_disable_count));
            enable_irq(poProximityRec->irq);
            atomic_dec( &prox_irq_disable_count );
        }
        DEBUG_LOG("irq_disable_count = %d",atomic_read(&prox_irq_disable_count));

        atomic_set(&updata_flag, 1);
...
static int IOECS_Disable(void)
{
    ...
    if(atomic_read(&enable_mode) == 1) {
        ...
        while( 0 < atomic_read(&prox_irq_enable_wake_count) ){
	    DEBUG_LOG("enable_wake_count = %d",atomic_read(&prox_irq_enable_wake_count));
            disable_irq_wake(poProximityRec->irq);
            atomic_dec( &prox_irq_enable_wake_count );
        }
        DEBUG_LOG("enable_wake_count = %d",atomic_read(&prox_irq_enable_wake_count));

        disable_irq(poProximityRec->irq);
        atomic_inc( &prox_irq_disable_count );
        DEBUG_LOG("irq_disable_count = %d",atomic_read(&prox_irq_disable_count));

        irq_set_irq_type(poProximityRec->irq, IRQ_TYPE_LEVEL_LOW);

        nResult = shdisp_api_prox_sensor_pow_ctl(SHDISP_PROX_SENSOR_BGMODE_OFF,NULL);
        if(nResult != SHDISP_RESULT_SUCCESS) {
...
int PROX_recovery_end_func(void)
{
    ...
    if (atomic_read(&updata_flag) == 1) {
        schedule_work(&prox_updata_val_work);
...
static int PROX_Probe(void)
{
    ...
    /* IRQ */
    nResult = request_irq(poProximityRec->irq, PROX_interrupt, IRQF_TRIGGER_LOW,
